...
function getLabel(date: string, period: TimePeriod): string {
  ...
  if (period === "day") return date.slice(5); // MM-DD
  if (period === "month") return `${d.getMonth() + 1}月`;
...
function PieTooltip({ active, payload }: any) {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg shadow-lg p-3 text-sm">
      ...
      <p className="text-gray-600">{formatAmount(value)}</p>
    </div>
  );
}
...
function StatCard({
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
      ...
      <p className={`text-2xl font-bold ${color}`}>{formatAmount(value)}</p>
    </div>
  );
}
...
export default function AnalyticsPage() {
  ...
  const transactions = mockTransactions; // 之後換成 API fetch
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          <StatCard label="總收入" value={totalIncome} color="text-green-600" />
          <StatCard label="總支出" value={totalExpense} color="text-red-500" />
          <StatCard
            label="淨收支"
            value={net}
          ...
          <div className="flex flex-wrap gap-3 items-center justify-between">
            ...
            <div className="flex gap-1 bg-gray-100 rounded-lg p-1">
              {(["bar", "line", "pie"] as ChartType[]).map((type) => {
                const labels = { bar: "長條圖", line: "折線圖", pie: "圓餅圖" };
                return (
                  <button
                    key={type}
                    ...
                    <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${pct}%`,
                          backgroundColor:
                            CATEGORY_COLORS[item.name] ?? "#94a3b8",
                        }}
                      />
                    </div>
                  </div>
